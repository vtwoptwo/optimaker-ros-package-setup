...
def quaternion_to_euler(data: PoseStamped):
    ...
    y = data.pose.orientation.y * 100
    z = data.pose.orientation.z * 100
...
def has_position_changed(new_position, threshold=0.1):
...
def listener():
    ...
    rospy.Subscriber("/mocap_node/Vera_Boss/pose", PoseStamped, callback)
    rospy.spin()
